...
# import data
rec<- read.csv("data/rec4shiny_2021.csv")
#rec$Year<-as.numeric(rec$Year)
rel<- read.csv("data/rel4shiny_2021.csv")
...
server <- function(input, output, session) {
  ...
  filteredData <- reactive( rec %>% filter(Year <= max(input$choices) & Year>=min(input$choices)) )
  filteredData2 <- reactive( rel %>% filter(Year <= max(input$choices) & Year>=min(input$choices)) )
