...
# PG additions 6/9/22
ybus2$GEN[ybus2$GEN %in% c("GEO.d","GEO.u")] <- "GEO.u" #Georgiana Slough
ybus2$GEN[ybus2$GEN %in% c("LPS.d", "LPS.u")] <- "LPS.u" #Little Potato Slough
ybus2$GEN[ybus2$GEN %in% c("MOK.d", "MOK.u")] <- "MOK.u"
# u/d is upstream/downstream, both in center of channel (chose up)
# already renamed these (lines 54-57), but do we want to give them a general name ("YB_BCE",               "YB_BCE2","YB_BCW","YB_BCW2")? They are near "CCH.1" & "CCH.2"...
# Sacramento river between Clarksburg and Sutter
ybus2$GEN[ybus2$GEN %in% c("1A", "1C")] <- "1A"
ybus2$GEN[ybus2$GEN %in% c("2A", "2B", "2C", "2D")] <- "2A" # closest to Clarksburg
ybus2$GEN[ybus2$GEN %in% c("3A", "3B", "3C", "3D")] <- "3A"
ybus2$GEN[ybus2$GEN %in% c("5A", "5B", "5C", "5D")] <- "5A" # just above entrance to Sutter
